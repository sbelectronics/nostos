; Bootstrap for interrupt-driven Z180 ASCI + CompactFlash + Z180 MMU.
; Channels 0 and 1 are interrupt-driven via the Z180 internal
; vectored interrupt mechanism (IM 2).  CF is the block device.
; Memory mapping uses the Z180 internal MMU (-DMAPPER_Z180_MMU).
;
...
; --- Build-flag assertions (see mapper_config.asm for sentinel contract) ---
    IFNDEF MAPPER_Z180_MMU_CHOSEN
        ERROR "512k-z180-mmu-int-cf bootstrap requires -DMAPPER_Z180_MMU"
    ENDIF
