...
;
; NOSTOS MEMORY TOP - below kernel stack
;
; *************************************
; *                                   *
; *           PROGRAM START           *
...
;
; NostOS relocatable header (13 bytes reserved)
	DEFS 13, 0
;
; *************************************
; *        CONFIGURATION DATA         *
...
CPMLLF: DB 0		;1 FOR LF AFTER CR ON LIST (NOT USED)
...
ITRMST: DB 0		;INIT TERMINAL (NO OP ON NOSTOS)
	DS 33-($-ITRMST)
RTRMST: DB 0		;RESET TERMINAL
	DS 33-($-RTRMST)
BSTLIN: DB 0		;BEGIN STATUS LINE STRING (NO STATUS LINE)
	DS 33-($-BSTLIN)
ESTLIN: DB 0		;END STATUS LINE STRING (NO STATUS LINE)
	DS 33-($-ESTLIN)
PSTUPS: DB 0		;PRINTER SETUP STRING
	DS 33-($-PSTUPS)
TWODSK: DB 0		;SET TO 1 IF TWO DISKS ARE AVAILABLE
;
; NostOS: game data filename
ZORKFN: DEFM "ZORK1.DAT"
	DEFB 0
;
; NostOS: file handle for game data
ZORKFH: DB 0
;
; *************************************
; *           NostOS START            *
...
;
; ------------------------------------------------------------
; PCSTR - print counted string (count in first byte)
; Uses NostOS DEV_CWRITE
; ------------------------------------------------------------
...
; PCPST - printer setup string (stubbed, no printer on NostOS)
PCPST:	RET

; SETFIL - not needed on NostOS (single file)
SETFIL: RET

; ------------------------------------------------------------
; GETDSK - read a 512-byte page from the game data file
; NostOS version: BSEEK to page, then BREAD into buffer
; Input:  A = page number, HL = destination buffer
; Output: data read into (HL), DBUFP advanced by 512
; ------------------------------------------------------------
GETDSK: LD (DBUFP),HL
	; BSEEK to page number A
	LD E,A
	LD D,0		;DE = block number
	LD A,(ZORKFH)
	LD B,A		;B = file handle
	LD C,DEV_BSEEK
	CALL KERNELADDR
	OR A
	CALL NZ,ZER	;SEEK ERROR -> RESTART
	...
	CALL KERNELADDR
	OR A
	CALL NZ,ZER	;READ ERROR -> RESTART
	; Advance DBUFP by 512
	LD HL,(DBUFP)
	LD DE,512
	ADD HL,DE
	LD (DBUFP),HL
...
; ------------------------------------------------------------
; CIN - read one character from console (blocking)
; NostOS version: uses DEV_CREAD_RAW
; Output: A = character (bit 7 stripped)
; ------------------------------------------------------------
CIN:	PUSH BC
	PUSH DE
	PUSH HL
...
; LPTO - printer output (stubbed, no printer on NostOS)
LPTO:	RET
...
; ------------------------------------------------------------
; SAVE - save game state (NOT IMPLEMENTED on NostOS)
; ------------------------------------------------------------
SAVE:
	LD HL,SAVMSG
	CALL MSGOUT
	...
	XOR A
	LD (ZPCFLG),A
	JP PREDF	;INDICATE FAILURE

SAVMSG: DEFM "Save not yet implemented."
	DEFB 0
...
; ------------------------------------------------------------
; MSGOUT - print null-terminated string at HL to console
; Uses NostOS DEV_CWRITE
; ------------------------------------------------------------
MSGOUT: LD C,(HL)
	LD A,(HL)
...
; ------------------------------------------------------------
; RESTOR - restore game state (NOT IMPLEMENTED on NostOS)
; ------------------------------------------------------------
RESTOR:
	LD HL,RSTMSG
	CALL MSGOUT
...
RSTMSG: DEFM "Restore not yet implemented."
	DEFB 0

; NostOS: single file, already open. These are stubs.
LFILS:	PUSH AF
	LD A,3
	LD (FILTYP),A	;MARK AS DAT FILE OPEN
	JP LFILR
...
;
; OPEN GAME DATA FILE VIA NostOS
;
	LD DE,ZORKFN
	...
	CP ERR_SUCCESS
	JP NZ,OPNERR
	LD A,L		;L = file handle
	LD (ZORKFH),A	;SAVE HANDLE
	LD A,3
	LD (FILTYP),A	;MARK AS DAT FILE OPEN
	JP BODY2
OPNERR:
	...
	LD C,SYS_EXIT
	CALL KERNELADDR
OPNMSG: DEFM "Cannot open ZORK1.DAT"
	DEFB 0
BODY2:
...
;
; FILE ALREADY OPEN VIA NostOS (ZORKFH has handle)
;
;
; INITIALIZE TERMINAL
;
...
;
LMEMRY:			;LAST LOCATION IN MEMORY (label = end of binary)
