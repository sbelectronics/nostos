...
;
; Changes to support Nostos are (C) 2026 Scott Baker
;
...
; WRKSPC label is placed at end of binary so workspace starts immediately after code
...
BRTEFG EQU WRKSPC+04EH ; Break test enable flag (0=off, 1=on)
RINPUT EQU WRKSPC+04FH ; Input reflection
STRSPC EQU WRKSPC+052H ; Bottom of string space
LINEAT EQU WRKSPC+054H ; Current line number
BASTXT EQU WRKSPC+056H ; Pointer to start of program
BUFFER EQU WRKSPC+059H ; Input buffer
STACK EQU WRKSPC+05EH ; Initial stack
CURPOS EQU WRKSPC+0A3H ; <<<< Character position on line ** Top of Loader TEMPSTACK **
LCRFLG EQU WRKSPC+0A4H ; Locate/Create flag
TYPE EQU WRKSPC+0A5H ; Data type flag
DATFLG EQU WRKSPC+0A6H ; Literal statement flag
LSTRAM EQU WRKSPC+0A7H ; Last available RAM
TMSTPT EQU WRKSPC+0A9H ; Temporary string pointer
TMSTPL EQU WRKSPC+0ABH ; Temporary string pool
TMPSTR EQU WRKSPC+0B7H ; Temporary string
STRBOT EQU WRKSPC+0BBH ; Bottom of string space
CUROPR EQU WRKSPC+0BDH ; Current operator in EVAL
LOOPST EQU WRKSPC+0BFH ; First statement of loop
DATLIN EQU WRKSPC+0C1H ; Line of current DATA item
FORFLG EQU WRKSPC+0C3H ; "FOR" loop flag
LSTBIN EQU WRKSPC+0C4H ; Last byte entered
READFG EQU WRKSPC+0C5H ; Read/Input flag
BRKLIN EQU WRKSPC+0C6H ; Line of break
NXTOPR EQU WRKSPC+0C8H ; Next operator in EVAL
ERRLIN EQU WRKSPC+0CAH ; Line of error
CONTAD EQU WRKSPC+0CCH ; Where to CONTinue
PROGND EQU WRKSPC+0CEH ; End of program
VAREND EQU WRKSPC+0D0H ; End of variables
ARREND EQU WRKSPC+0D2H ; End of arrays
NXTDAT EQU WRKSPC+0D4H ; Next data item
FNRGNM EQU WRKSPC+0D6H ; Name of FN argument
FNARG EQU WRKSPC+0D8H ; FN argument value
FPREG EQU WRKSPC+0DCH ; Floating point register
FPEXP EQU FPREG+3     ; Floating point exponent
SGNRES EQU WRKSPC+0E0H ; Sign of result
PBUFF EQU WRKSPC+0E1H ; Number print buffer
MULVAL EQU WRKSPC+0EEH ; Multiplier
PROGST EQU WRKSPC+0F1H ; Start of program text area
STLOOK EQU WRKSPC+155H ; Start of memory test
...
UF EQU 22H ; UnDEFined user function FN
...
; BASIC CODE COMMENCES
    ...
    INCLUDE "../../src/include/syscall.asm"
        ...
        ORG     0               ; Relocatable (processed by mkreloc.py)
...
COLD:   JP      CSTART          ; Jump in for cold start
...
INIT:   LD      HL,INITAB       ; Initialise workspace
        ...
        ; Copy BC bytes from HL to DE (8080-compatible LDIR)
INITCP: LD      A,(HL)
        LD      (DE),A
        INC     HL
        INC     DE
        ...
        ; Get top of memory from NostOS
        LD      C, SYS_MEMTOP
        CALL    KERNELADDR      ; HL = top of user memory
...
SIGNON: DEFB    "Nostos BASIC Ver 4.7c",CR,LF
...
DELCHR: LD      A,BKSP          ; Backspace - erase char on VT100
        CALL    bas_cout        ; Send BS directly (bypass OUTC)
        LD      A,' '           ; Space overwrites character
        CALL    bas_cout        ; Send space directly
        LD      A,BKSP          ; Backspace again
        CALL    bas_cout        ; Send BS directly
        LD      A,(CURPOS)      ; Fix cursor position counter
        DEC     A
        LD      (CURPOS),A
        DEC     HL              ; Back space buffer
...
MORINP: CALL    CLOTST          ; Get character and test ^O
        ...
        JP      Z,DELCHR        ; Yes - Erase character (VT100 style)
...
DINPOS: POP     AF              ; Restore character
        POP     BC              ; Restore buffer length
        JP      bas_cout        ; Send character in A to console via NostOS

OUTNCR: CALL    OUTC            ; Output character in A
        JP      PRNTCRLF        ; Output CRLF

CLOTST: CALL    bas_cin         ; Get input character via NostOS
        AND     01111111B       ; Strip bit 7
...
COUNT:  PUSH    HL              ; Save code string address
        ...
        CALL    bas_cin         ; Get input character via NostOS
        CP      CTRLC           ; Is it control "C"?
...
TSTBRK: LD      A,(BRTEFG)      ; Check if break checking enabled
        OR      A
        RET     Z               ; Disabled - skip check
        CALL    bas_cstat       ; Check input status via NostOS
        OR      A               ; Check count Zero
        RET     Z               ; No key, go back
        CALL    bas_cin         ; Get the key into A via NostOS
        CP      ESC             ; Escape key?
        JP      Z,BRK           ; Yes, break
        CP      CTRLC           ; <Ctrl-C>
        JP      Z,BRK           ; Yes, break
...
STALL:  CALL    bas_cin         ; Wait for key via NostOS
...
NORMAL: DEC     B               ; Count bits
        ...
        LD      A,D             ; Get NMSB, shift left with last bit
        RLA
        LD      D,A
        LD      A,C             ; Get MSB, shift left with last bit
        RLA
        LD      C,A
        OR      A               ; Set sign flag for JP P test
...
SHRLP:  XOR     A               ; Flag for all done
        ...
        RET     Z               ; Yes - Return
SHRT1:  LD      A,C             ; Get MSB, shift it right
        RRA
        LD      C,A
        LD      A,D             ; Get NMSB,shift right with last bit
        RRA
        LD      D,A
        LD      A,E             ; Get LSB, shift right with last bit
        RRA
        LD      E,A
        LD      A,B             ; Get underflow, shift right with last bit
        RRA
        LD      B,A
...
RESDIV: POP     BC              ; Restore divisor
        ...
        RLA                     ; Restore carry
        LD      A,E             ; Get LSB of quotient, double it
        RLA
        LD      E,A
        LD      A,D             ; Get NMSB of quotient, double it
        RLA
        LD      D,A
        LD      A,C             ; Get MSB of quotient, double it
        RLA
        LD      C,A
        ADD     HL,HL           ; Double NMSB,LSB of divisor
        LD      A,B             ; Get MSB of divisor, double it
        RLA
        LD      B,A
...
MLDEBC:                         ; Multiply DE by BC to HL
        ...
        RET     Z               ; Return zero if zero
        LD      A,B
        LD      (DIV4),A        ; Save high byte of multiplier
        LD      B,16            ; 16 bits (iterations)
MLDBLP: ADD     HL,HL           ; Shift partial product left
        JP      C,BSERR         ; ?BS Error if overflow
        LD      A,C             ; Shift low byte of multiplier left
        ADD     A,A
        LD      C,A
        LD      A,(DIV4)        ; Get high byte of multiplier
        RLA                     ; Rotate left through carry from C
        LD      (DIV4),A        ; Save updated high byte
...
BREAK:  CALL    GETNUM          ; Get a number
        CALL    DEINT           ; Get integer -32768 to 32767
        LD      A, E            ; 0 = off, non-zero = on
        LD      (BRTEFG), A     ; Set break test enable flag
        RET
...
MONITR: LD      C, SYS_EXIT     ; Return to NostOS executive
        CALL    KERNELADDR
        RET                     ; Fallback (should not reach here)
...
MEEKLLP:
        ...
        DEC     B
        JP      NZ,MEEKLLP      ; Do 16 byte blocks
...
MEEKDOT:
        ...
        INC     HL              ; Get next address
        DEC     B
        JP      NZ,MEEKASC      ; Do 16 byte blocks
...
HLD_WAIT_COLON:
        call    bas_cin         ; Rx byte in A via NostOS
        cp ':'                  ; wait for ':'
...
; ============================================================
; NostOS File I/O Commands (SAVE / LOAD)
; ============================================================

; ------------------------------------------------------------
; BSAVE - Save program as ASCII text to a file
; Usage: SAVE "filename"
; ------------------------------------------------------------
BSAVE:
        CALL    EVAL            ; Evaluate string expression
        PUSH    HL              ; Save code string pointer
        CALL    GETSTR          ; Verify it's a string; HL = descriptor
        LD      A, (HL)         ; Get length from descriptor
        OR      A
        JP      Z, BSAVE_ERR    ; Empty filename - error
        LD      C, A            ; C = remaining length
        INC     HL
        INC     HL
        LD      E, (HL)         ; Get LSB of string address
        INC     HL
        LD      D, (HL)         ; Get MSB of string address
        ; DE = string data, C = length
        LD      HL, bas_fname
BSAVE_CP:
        LD      A, (DE)
        CP      'a'             ; Upcase a-z
...
BSAVE_NUC:
        ...
        ; File exists - remove and retry (re-parse since PATH_WORK may be clobbered)
        CP      ERR_EXISTS
        JP      NZ, BSAVE_ERR
        LD      DE, bas_fname
        LD      C, SYS_PATH_PARSE
...
BSAVE_OPENED:
        LD      A, L
        LD      (bas_file_dev), A   ; Save file handle
        ; Initialize write buffer position and file size counter
        LD      HL, 0
        LD      (bas_wbuf_pos), HL
        LD      (bas_filesize), HL
        LD      (bas_filesize + 2), HL

        ; Walk program and write each line as ASCII
        LD      HL, (BASTXT)
BSAVE_LINE:
        LD      C, (HL)             ; LSB of next line ptr
        INC     HL
        LD      B, (HL)             ; MSB of next line ptr
        INC     HL
        LD      A, B
        OR      C
        JP      Z, BSAVE_DONE      ; End of program
        ...
        LD      D, (HL)
        INC     HL
        PUSH    HL                  ; Save line body address
        ...
        POP     HL                  ; Restore line body

        ; De-tokenize and write line
BSAVE_LBODY:
        LD      A, (HL)
        OR      A
        JP      Z, BSAVE_EOL       ; End of line
        INC     HL
        JP      P, BSAVE_LCHAR     ; Not a token - write directly

        ; Token: expand to keyword
        PUSH    HL
        SUB     ZEND-1              ; Token offset
        LD      C, A
        LD      DE, WORDS
BSAVE_FTOK:
        LD      A, (DE)
        INC     DE
        OR      A
        JP      P, BSAVE_FTOK      ; Skip until word start (bit 7)
        DEC     C
        JP      NZ, BSAVE_FTOK
        ; Found the keyword
BSAVE_FWORD:
        ...
        INC     DE
        OR      A
        JP      P, BSAVE_FWORD     ; More chars in keyword
        POP     HL
        JP      BSAVE_LBODY
...
BSAVE_DONE:
        CALL    bas_fflush      ; Flush remaining bytes in write buffer
        ...
        CALL    KERNELADDR
        CALL    bas_close_file
        POP     HL              ; Restore code string pointer
        RET
...
; ------------------------------------------------------------
; BLOAD - Load an ASCII program from a file
; Usage: LOAD "filename"
; ------------------------------------------------------------
BLOAD:
        CALL    EVAL            ; Evaluate string expression
...
BLOAD_NUC:
        ...
        ; Clear program (like NEW)
        CALL    CLRPTR

        ; Initialize read buffer: set pos=512 to trigger first block read
        LD      HL, 512
        LD      (bas_wbuf_pos), HL

        ; Read lines from file and insert them
BLOAD_LINE:
        ; Read a line into bas_lbuf
        LD      HL, bas_lbuf
        LD      B, 0                ; Character count
BLOAD_RCHAR:
        PUSH    HL
        PUSH    BC
        CALL    bas_fgetc           ; A = char, carry = EOF
        POP     BC
        POP     HL
        JP      C, BLOAD_EOF        ; EOF or read error

        CP      0                   ; Null byte = end of real content
        JP      Z, BLOAD_EOF
        CP      LF                  ; Skip LF
        JP      Z, BLOAD_RCHAR
        CP      CR                  ; End of line
...
BLOAD_GOT:
        ...
        LD      A, B
        OR      A
        JP      Z, BLOAD_LINE       ; Skip empty lines

        ; Process the line: tokenize and insert
        LD      HL, bas_lbuf-1       ; GETCHR will increment
        CALL    GETCHR               ; Get first character
        JP      Z, BLOAD_LINE        ; Empty after skip

        PUSH    AF                   ; Save carry (digit = carry set)
        CALL    ATOH                 ; Get line number into DE
        PUSH    DE                   ; Save line number
        CALL    CRUNCH               ; Tokenize rest of line
        LD      B, A                 ; Length of tokenized line
        POP     DE                   ; Restore line number
        POP     AF                   ; Restore carry
        JP      NC, BLOAD_LINE       ; No line number - skip

        ; Insert line into program (replicate GETCMD logic)
        PUSH    DE
        PUSH    BC
        XOR     A
        LD      (LSTBIN), A
        CALL    GETCHR
        PUSH    AF
        CALL    SRCHLN               ; Search for line number
        JP      C, BLOAD_LFND
        POP     AF
        PUSH    AF
        JP      Z, BLOAD_SETPTR      ; Empty line body (delete only)
        OR      A                    ; Clear carry
BLOAD_LFND:
        PUSH    BC
        JP      NC, BLOAD_INEW
...
BLOAD_INEW:
        POP     DE                   ; Address of line
        POP     AF                   ; Status
        JP      Z, BLOAD_SETPTR
        LD      HL, (PROGND)
        EX      (SP), HL             ; Get length, save end
        POP     BC                   ; End of program to BC
        ADD     HL, BC
        PUSH    HL
        CALL    MOVUP                ; Make space
        POP     HL
        LD      (PROGND), HL
        EX      DE, HL
        LD      (HL), H              ; Placeholder next-ptr
        POP     DE                   ; Line number
        INC     HL
        INC     HL
        LD      (HL), E              ; Line number LSB
        INC     HL
        LD      (HL), D              ; Line number MSB
        INC     HL
        LD      DE, BUFFER
BLOAD_MBUF:
        LD      A, (DE)
...
BLOAD_PTRLP:
        ...
        INC     HL
        OR      (HL)
        JP      Z, BLOAD_LINE       ; Done fixing pointers - next line
        INC     HL
        INC     HL
        INC     HL
        XOR     A
...
BLOAD_EOF:
        ; Check if there's a partial line to process
        LD      A, B
        OR      A
        JP      NZ, BLOAD_GOT       ; Process remaining chars
        ; Close file and return to command mode
        CALL    bas_close_file
        JP      PRNTOK               ; Print "Ok" and go to command mode

BLOAD_ERR:
        JP      FCERR

; ------------------------------------------------------------
; bas_fputc - Write character A to the open file
; Preserves: BC, DE, HL
; ------------------------------------------------------------
; ------------------------------------------------------------
; bas_fputc - Write character A to the file write buffer
; Flushes to disk when buffer reaches 512 bytes.
; Preserves: BC, DE, HL
; ------------------------------------------------------------
bas_fputc:
        PUSH    HL
        ...
        ; Store byte into wbuf[wbuf_pos]
        LD      HL, (bas_wbuf_pos)
        LD      DE, bas_wbuf
        ADD     HL, DE              ; HL = &wbuf[pos]
        LD      (HL), C
        ; Increment file size counter (3 bytes)
        LD      HL, bas_filesize
        INC     (HL)
        JP      NZ, bas_fputc_noinc
        INC     HL
        INC     (HL)
        JP      NZ, bas_fputc_noinc
        INC     HL
...
bas_fputc_noinc:
        ...
        INC     HL
        LD      (bas_wbuf_pos), HL
        ; Check if pos == 512 (H == 2, L == 0)
        LD      A, H
        CP      2
        JP      NZ, bas_fputc_done
        LD      A, L
        OR      A
        JP      NZ, bas_fputc_done
        ; Buffer full: flush to disk
        CALL    bas_fflush_block
bas_fputc_done:
        POP     BC
        POP     DE
        POP     HL
...
; ------------------------------------------------------------
; bas_fflush_block - Write full 512-byte block to file
; Resets wbuf_pos to 0.
; ------------------------------------------------------------
bas_fflush_block:
        LD      A, (bas_file_dev)
        LD      B, A
...
; ------------------------------------------------------------
; bas_fflush - Flush any remaining bytes in write buffer
; Pads remainder of block with zeros, then writes.
; Only flushes if wbuf_pos > 0.
; ------------------------------------------------------------
bas_fflush:
        LD      HL, (bas_wbuf_pos)
        LD      A, H
        ...
        LD      DE, bas_wbuf
        ADD     HL, DE          ; HL = &wbuf[pos]
        ; Calculate remaining = 512 - pos
        LD      A, (bas_wbuf_pos)
        LD      C, A
        LD      A, 0
        SUB     C
        LD      C, A            ; C = 256 - pos_low (or 0 if pos_low=0)
        LD      A, (bas_wbuf_pos+1)
        LD      B, A
        LD      A, 1
        SBC     A, B
        LD      B, A            ; BC = 512 - pos
        ; If BC == 0 (pos was exactly 512), skip fill
        LD      A, B
        OR      C
        JP      Z, bas_fflush_wr
bas_fflush_zf:
...
; ------------------------------------------------------------
; bas_fgetc - Read one character from file via block buffer
; If wbuf_pos >= 512, reads next 512-byte block from file.
; Outputs:
;   A  - character read (if carry clear)
;   Carry set on read error (EOF)
; Preserves: BC, DE, HL
; ------------------------------------------------------------
bas_fgetc:
        PUSH    HL
        ...
        LD      A, H
        CP      2                   ; pos >= 512?
        JP      C, bas_fgetc_rd     ; No, still have data
        ; Read next block
        LD      A, (bas_file_dev)
        LD      B, A
        LD      DE, bas_wbuf
        ...
        CALL    KERNELADDR
        CP      ERR_SUCCESS
        JP      NZ, bas_fgetc_eof   ; Read error/EOF
        LD      HL, 0
        LD      (bas_wbuf_pos), HL
bas_fgetc_rd:
        ; Read byte from wbuf[pos]
        LD      DE, bas_wbuf
        ADD     HL, DE              ; HL = &wbuf[pos]
        LD      A, (HL)             ; A = character
        ; Increment pos
...
; ------------------------------------------------------------
; bas_fput_dec16 - Write HL as decimal number to file
; Preserves: nothing important (called with line# in HL)
; ------------------------------------------------------------
bas_fput_dec16:
        PUSH    HL
        ; Build decimal string right-to-left in bas_decbuf
        LD      DE, bas_decbuf + 5
        LD      A, 0
...
BFPD_LOOP:
        ...
        OR      L
        JP      NZ, BFPD_LOOP
        ; DE points to first digit, write string
BFPD_OUT:
        LD      A, (DE)
        OR      A
        JP      Z, BFPD_DONE
...
; ------------------------------------------------------------
; bas_close_file - Close the current file handle
; ------------------------------------------------------------
bas_close_file:
        LD      A, (bas_file_dev)
        LD      B, A
...
; File I/O data
bas_file_dev:   DEFB 0              ; Current file device handle
bas_fname:      DEFS 17, 0          ; Filename buffer (16 chars + null)
bas_decbuf:     DEFS 6, 0           ; Decimal conversion buffer
bas_lbuf:       DEFS 74, 0          ; Line input buffer for LOAD (72+null+spare)
bas_wbuf_pos:   DEFS 2, 0           ; Write buffer position (0-511)
bas_filesize:   DEFS 4, 0           ; Accumulated file size for BSETSIZE
bas_wbuf:       DEFS 512, 0         ; Write buffer for SAVE

; ============================================================
; NostOS I/O Helper Routines
; These replace the RST-based I/O of the original BASIC
; ============================================================

; ------------------------------------------------------------
; bas_cout - Output character in A to console
...
; ------------------------------------------------------------
; bas_cin - Read character from console (blocking)
; Inputs:  none
; Outputs: A = character read
; Preserves: BC, DE, HL
; ------------------------------------------------------------
bas_cin:
        PUSH    HL
        ...
        LD      C, DEV_CREAD_RAW
        CALL    KERNELADDR
        LD      A, L            ; DEV_CREAD_RAW returns char in L
        POP     BC
        POP     DE
        POP     HL
        RET
...
; ------------------------------------------------------------
; bas_cstat - Check console input status
; Inputs:  none
; Outputs: A = number of characters waiting
; Preserves: BC, DE, HL
; ------------------------------------------------------------
bas_cstat:
        PUSH    HL
        ...
        LD      C, DEV_STAT
        CALL    KERNELADDR
        LD      A, L            ; DEV_STAT returns count in L
        POP     BC
        POP     DE
        POP     HL
        RET
